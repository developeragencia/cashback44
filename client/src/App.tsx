...
import FinancialDashboard from "@/pages/client/financial-dashboard";
import QRPaymentSystem from "@/pages/merchant/qr-payment-system";
import CompleteAdminPanel from "@/pages/admin/complete-admin-panel";
import { ModernPWAInstall } from "@/components/ui/modern-pwa-install";

// Auth Pages
import Login from "@/pages/auth/login";
import WelcomeLogin from "@/pages/auth/welcome-login";
...
import AdminUsersSimple from "@/pages/admin/users-simple";
...
import WelcomeNewPage from "@/pages/welcome-new";
import WelcomeSimplePage from "@/pages/welcome-simple";
import WelcomeEnhancedPage from "@/pages/welcome-enhanced";

function Router() {
  // Vamos simplificar completamente a lógica de roteamento para evitar conflitos
  
  // Rota atual
  const currentPath = window.location.pathname;
  console.log("Rota atual detectada:", currentPath);
  
  // Renderização direta das páginas de boas-vindas sem redirecionamentos complexos
  if (currentPath === '/welcome-simple') {
    console.log("Renderizando página simple");
    return <WelcomeSimplePage />;
  } 
  
  if (currentPath === '/welcome-new') {
    console.log("Renderizando página new");
    return <WelcomeNewPage />;
  }
  
  if (currentPath === '/welcome-static' || currentPath === '/welcome') {
    console.log("Renderizando página static");
    return <WelcomeStaticPage />;
  }
  
  // Se estamos na raiz, mostre a nova página de boas-vindas aprimorada
  if (currentPath === '/') {
    console.log("Renderizando página de boas-vindas aprimorada na raiz");
    return <WelcomeEnhancedPage />;
  }
  
  // Para forçar teste da tela de boas-vindas
  if (currentPath === '/welcome-force') {
    localStorage.removeItem('has_seen_welcome');
    console.log("Forçando visualização da nova página de boas-vindas");
    return <WelcomeEnhancedPage />;
  }
  ...
  return (
    <Switch>
      ...
      <Route path="/welcome-enhanced" component={WelcomeEnhancedPage} />
      <Route path="/" component={WelcomeEnhancedPage} />
      ...
      <Route path="/welcome" component={WelcomeStaticPage} />
      <Route path="/welcome-static" component={WelcomeStaticPage} />
      <Route path="/welcome-force" component={WelcomeStaticPage} />
      ...
      <ProtectedRouteMobile path="/client/financial-dashboard" component={FinancialDashboard} userType="client" title="Dashboard Financeiro" />
      ...
      <ProtectedRouteMobile path="/merchant/payment/:id" component={React.lazy(() => import('@/pages/merchant/payment'))} userType="merchant" title="Detalhes do Pagamento" />
      <ProtectedRouteMobile path="/merchant/payment" component={React.lazy(() => import('@/pages/merchant/payment'))} userType="merchant" title="Detalhes do Pagamento" />
      ...
      <ProtectedRouteMobile path="/merchant/withdrawals-new" component={MerchantWithdrawals} userType="merchant" title="Solicitar Saque" />
      <ProtectedRouteMobile path="/merchant/withdrawals" component={MerchantWithdrawals} userType="merchant" title="Solicitar Saque" />
      <ProtectedRouteMobile path="/merchant/salaries" component={MerchantSalaries} userType="merchant" title="Comissões e Taxas" />
      <ProtectedRouteMobile path="/merchant/qr-payment-system" component={QRPaymentSystem} userType="merchant" title="Sistema de Pagamento QR" />
      ...
      <ProtectedRouteMobile path="/admin/users" component={AdminUsersSimple} userType="admin" title="Usuários" />
      <ProtectedRouteMobile path="/admin/users-complex" component={AdminUsers} userType="admin" title="Usuários Completo" />
      ...
      <ProtectedRouteMobile path="/admin/complete-panel" component={CompleteAdminPanel} userType="admin" title="Painel Completo" />
...
function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const isPublicPage = 
        ...
        currentPath === '/welcome' ||
        currentPath === '/welcome-static'; // Adicionando as rotas welcome como públicas
      
      if (isPublicPage) {
        console.log("Página pública detectada, ignorando redirecionamento de autenticação");
        setLoadingAPI(false);
        return;
      }
  ...
  // Verificar se estamos na raiz ou em página de boas-vindas
  const currentPath = window.location.pathname;
  const hasSeenWelcome = localStorage.getItem('has_seen_welcome');
  const shouldShowWelcome = 
    (currentPath === '/' && !hasSeenWelcome) || 
    currentPath === '/welcome' || 
    currentPath === '/welcome-static' || 
    currentPath === '/welcome-enhanced' ||
    currentPath === '/welcome-simple' ||
    currentPath === '/welcome-new' ||
              ...
              <TooltipProvider>
                ...
                {showSplash ? (
                  ...
                  <div className="min-h-screen">
                    {/* Se estamos na página raiz ou welcome, mostrar diretamente a página de boas-vindas */}
                    {shouldShowWelcome ? (
                      <WelcomeEnhancedPage />
                    ) : (
                      <>
