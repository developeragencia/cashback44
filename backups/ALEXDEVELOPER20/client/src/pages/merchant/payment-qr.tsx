...
export default function MerchantPaymentQR() {
  ...
  const generateQRMutation = useMutation({
    ...
    onSuccess: (data) => {
      // Dados originais do servidor já devem estar no formato correto
      console.log("QR code dados recebidos do servidor:", data);
      
      // Garantindo que o valor é um número
      data.amount = typeof data.amount === 'number' ? data.amount : parseFloat(String(data.amount)) || 0;
      
      // Definir o estado com os dados processados
      setQrCodeData(data);
      setPaymentStatus(null);
      
      // Calcular quando o QR code expira (30 minutos a partir de agora)
      ...
      toast({
        ...
        description: "O código ficará ativo por 30 minutos.",
      });
      
      // Iniciar a verificação do status
      queryClient.invalidateQueries({ queryKey: ['/api/merchant/payment-qr', data.qr_code_id] });
    },
    onError: (error: any) => {
      toast({
