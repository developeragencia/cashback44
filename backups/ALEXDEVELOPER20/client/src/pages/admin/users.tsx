...
export default function AdminUsers() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['/api/admin/users', searchTerm, filterType, filterStatus, currentPage],
    queryFn: async () => {
  ...
  // Estados para controlar carregamento/processamento
  ...
  const handleSubmitEditForm = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiRequest('PATCH', `/api/admin/users/${editForm.id}`, {
        name: editForm.name,
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const response = await apiRequest('POST', `/api/admin/users/${selectedUser.id}/reset-password`, {
        newPassword
      });
  ...
  const handleBlockUser = async () => {
    ...
    try {
      ...
      const response = await apiRequest('PATCH', `/api/admin/users/${selectedUser.id}/status`, {
        status: newStatus
      });
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        throw new Error(error.message || "Erro ao modificar o status do usuário");
      }
    } catch (error) {
      toast({
        title: "Erro",
    ...
    <DashboardLayout title="Gestão de Usuários" type="admin">
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="font-medium text-lg">Gerenciamento de Usuários</h2>
            <Button 
              variant="outline" 
              onClick={() => {
                setSearchTerm('');
      ...
      <Dialog open={isResetPasswordDialogOpen} onOpenChange={setIsResetPasswordDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Você está prestes a redefinir a senha do usuário {selectedUser?.name}.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleSubmitEditForm} className="space-y-4 py-4">
            <div className="space-y-2">
