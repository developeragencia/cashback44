...
// Compat: endpoint usado pelo frontend para validar sessão
app.get('/api/auth/me', (req, res) => {
  if (!(req.session && req.session.userId)) {
    return res.status(401).json({ message: 'Não autenticado' });
  }
  const testUsers = {
    1: { id: 1, name: 'Administrador', email: 'admin@valecashback.com', type: 'admin' },
    2: { id: 2, name: 'Cliente Teste', email: 'cliente@valecashback.com', type: 'client' },
    3: { id: 3, name: 'Lojista Teste', email: 'lojista@valecashback.com', type: 'merchant' }
  };
  const user = testUsers[req.session.userId];
  if (!user) {
    return res.status(404).json({ message: 'Usuário não encontrado' });
