    ...
    if (checkColumns.rows.length > 0) {
      ...
      await pool.query(`
        ...
        SET fee_percentage = 5.0,
            fee_amount = CASE 
                          WHEN fee_amount IS NULL THEN CAST(amount as NUMERIC) * 0.05
                          ELSE fee_amount
                         END,
            net_amount = CASE 
                          WHEN net_amount IS NULL THEN CAST(amount as NUMERIC) * 0.95
                          ELSE net_amount
                         END
      ...
      await pool.query(`
        UPDATE withdrawal_requests
        SET fee_percentage = 5.0,
            fee_amount = CAST(amount as NUMERIC) * 0.05,
            net_amount = CAST(amount as NUMERIC) * 0.95
      `);
