...
export default function ClientStores() {
  ...
  // Buscar todas as lojas (usando a API pública /api/stores)
  const { data: storesData, isLoading, error } = useQuery<StoreItem[]>({
    ...
    queryFn: async () => {
      try {
        console.log("Buscando lojas da API pública...");
        const response = await fetch('/api/stores', {
          credentials: 'include'
        });
        
        if (!response.ok) {
          console.error("Erro ao buscar lojas:", response.status, response.statusText);
        ...
        // Em ambiente de produção, retornar array vazio para mostrar UI de "Nenhuma loja cadastrada"
        if (process.env.NODE_ENV === 'production') {
          return [];
        }
        
        // Em ambiente de desenvolvimento, podemos mostrar algumas lojas de exemplo para testes de UI
        return [
          { 
            id: 999, 
            name: "Loja de Exemplo (Dev)", 
            store_name: "Loja de Exemplo (Dev)",
            category: "Teste", 
            address: "Apenas em Ambiente de Desenvolvimento", 
            city: "Cidade Teste", 
            state: "TE", 
            commissionRate: 2.5, 
            rating: 5.0, 
            createdAt: new Date().toISOString(), 
            email: "exemplo@teste.dev", 
            phone: "(00) 0000-0000", 
            transactions: 0,
            volume: 0
          }
