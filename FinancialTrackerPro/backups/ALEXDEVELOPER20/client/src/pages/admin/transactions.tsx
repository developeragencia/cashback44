...
export default function AdminTransactions() {
  ...
  const { data, isLoading, error } = useQuery<{
    ...
    placeholderData: {
      transactions: Array(10).fill(null).map((_, index) => ({
        id: index + 1,
        customer: {
        ...
        merchant: {
          ...
          name: `Loja ${Math.floor(Math.random() * 5) + 1}`,
          logo: "",
        },
        totalAmount: Math.random() * 1000,
        cashbackAmount: Math.random() * 20,
        paymentMethod: ["credit_card", "debit_card", "pix", "cash"][Math.floor(Math.random() * 4)],
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        const transaction = row.original as Transaction;
        return renderStatus(transaction.status);
      },
    },
    {
      header: "Ações",
              ...
              <div className="space-y-4">
                <div className="border rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="text-sm font-medium">$ {transactionDetail.amount.subtotal.toFixed(2)}</div>
                    
                    {transactionDetail.amount.discount > 0 && (
                      <>
                        <div className="text-sm text-muted-foreground">Desconto:</div>
                        <div className="text-sm font-medium text-red-600">-$ {transactionDetail.amount.discount.toFixed(2)}</div>
                      </>
                    )}
                    ...
                    {transactionDetail.amount.tax > 0 && (
                      <>
                        ...
                        <div className="text-sm font-medium">$ {transactionDetail.amount.tax.toFixed(2)}</div>
                      </>
                    )}
                    
                    <div className="text-sm text-muted-foreground">Cashback:</div>
                    <div className="text-sm font-medium text-green-600">$ {transactionDetail.amount.cashback.toFixed(2)}</div>
                    
                    <div className="text-sm text-muted-foreground font-bold pt-2">Total:</div>
                    <div className="text-sm font-bold pt-2">$ {transactionDetail.amount.total.toFixed(2)}</div>
                  </div>
                </div>
                  ...
                  <table className="w-full">
                    ...
                    <tfoot>
                      <tr>
                        ...
                        <td className="text-right py-2 font-bold">$ {transactionDetail.amount.total.toFixed(2)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
